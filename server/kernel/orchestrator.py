        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                result_payload = {
                    ...
                    # monitor-facing signals
                    "_emotion":       ls.get("emotion", "neutral"),
                    "_engagement":    ls.get("engagement", 5.0),
                    "_misconception": ls.get("misconception", ""),
                    # agent observatory trace (observer.html, investor demo)
                    "_agent_trace": {
                        "read_trace":        ctx.module_data.get("learner.state.read_trace", []),
                        "dispatch_trace":    ctx.module_data.get("learner.state.dispatch_trace", []),
                        "agent_signals":     ctx.module_data.get("learner.state.agent_signals", {}),
                        "context_assembled": ctx.module_data.get("learner.state.context_assembled", ""),
                    },
                }
                ...
                # Write idempotency record — exclude agent trace (debug only, not needed for replay)
                idempotency_payload = {k: v for k, v in result_payload.items() if k != "_agent_trace"}
                await conn.execute(
                    """
                    ...
                    """,
                    uuid.UUID(request_id), json.dumps(idempotency_payload),
                )
