...
class ChallengeCalibratorAgent(BaseAgent):
    ...
    async def _observe(self, signals: AgentSignals, conn) -> None:
        ...
        max_turns = cfg.get("max_frustration_turns", 2)
        ...
        threshold = (row["frustration_threshold"] if row else float(max_turns)) or float(max_turns)
    ...
    async def _read(self, conn, learner_id: str) -> str:
        ...
        productive_failure = cfg.get("productive_failure", True)
        max_turns         = cfg.get("max_frustration_turns", 2)
        ...
        threshold  = row["frustration_threshold"] or float(max_turns)
        difficulty = row["difficulty_level"] or 0.5
        help_threshold = int(threshold) if productive_failure else 1
        if streaks >= help_threshold:
