...
_RETIREMENT_MASTERY = Config.ANALOGY_RETIREMENT_MASTERY
_MASTERY_THRESHOLD  = Config.ANALOGY_MASTERY_THRESHOLD


class TransferWeaverAgent(BaseAgent):
    ...
    async def _observe(self, signals: AgentSignals, conn) -> None:
        ...
        retirement_score = cfg.get("retirement_score", Config.ANALOGY_RETIREMENT_SCORE)
        ...
        if (mastered or 0) >= _RETIREMENT_MASTERY and scores[top] > retirement_score:
            scores[top] = 0.0
            await conn.execute(
    ...
    async def _read(self, conn, learner_id: str) -> str:
        ...
        failed = await conn.fetch(
            "SELECT domain FROM learner_state.analogy_history "
