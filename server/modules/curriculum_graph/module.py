...
class CurriculumGraphModule(Module):
    ...
    async def handle(self, event: Event, ctx: KernelContext) -> ModuleResult:
        ...
        # Run DB graph lookups sequentially — the shared asyncpg connection (ctx.db)
        # cannot handle concurrent queries when modules run in parallel.
        # retrieve_semantic uses httpx (not ctx.db) so is safe to run last.
        try:
            neighbours = await get_concept_neighbourhood(ctx.db, concept_ids)
        except Exception as e:
            log.warning("graph_query_failed", error=str(e))
            neighbours = []
        try:
            next_concept = await get_next_concept(ctx.db, ctx.learner_id, ctx.grade, ctx.subject)
        except Exception as e:
